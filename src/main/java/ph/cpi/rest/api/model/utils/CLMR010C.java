package ph.cpi.rest.api.model.utils;

public class CLMR010C {
	private String extractUser;
	private String extractDate;
	private String adjId;
	private String adjName;
	private String claimId;
	private String claimNo;
	private String policyId;
	private String policyNo;
	private String cedingId;
	private String cedingName;
	private String coRefNo;
	private String lossDate;
	private String lossCd;
	private String lossAbbr;
	private String insuredDesc;
	private String currencyCd;
	private String insuredClm;
	private String approvedAmt;
	private String lossOsAmt;
	private String lossPdAmt;
	private String adjFeeAmt;
	private String clmStatCd;
	private String clmStatDesc;
	private String adjIdParam;
	private String clmStatParam;
	private String dateParam;
	private String dateRange;
	private String dateFrom;
	private String dateTo;
	public String getExtractUser() {
		return extractUser;
	}
	public void setExtractUser(String extractUser) {
		this.extractUser = extractUser;
	}
	public String getExtractDate() {
		return extractDate;
	}
	public void setExtractDate(String extractDate) {
		this.extractDate = extractDate;
	}
	public String getAdjId() {
		return adjId;
	}
	public void setAdjId(String adjId) {
		this.adjId = adjId;
	}
	public String getAdjName() {
		return adjName;
	}
	public void setAdjName(String adjName) {
		this.adjName = adjName;
	}
	public String getClaimId() {
		return claimId;
	}
	public void setClaimId(String claimId) {
		this.claimId = claimId;
	}
	public String getClaimNo() {
		return claimNo;
	}
	public void setClaimNo(String claimNo) {
		this.claimNo = claimNo;
	}
	public String getPolicyId() {
		return policyId;
	}
	public void setPolicyId(String policyId) {
		this.policyId = policyId;
	}
	public String getPolicyNo() {
		return policyNo;
	}
	public void setPolicyNo(String policyNo) {
		this.policyNo = policyNo;
	}
	public String getCedingId() {
		return cedingId;
	}
	public void setCedingId(String cedingId) {
		this.cedingId = cedingId;
	}
	public String getCedingName() {
		return cedingName;
	}
	public void setCedingName(String cedingName) {
		this.cedingName = cedingName;
	}
	public String getCoRefNo() {
		return coRefNo;
	}
	public void setCoRefNo(String coRefNo) {
		this.coRefNo = coRefNo;
	}
	public String getLossDate() {
		return lossDate;
	}
	public void setLossDate(String lossDate) {
		this.lossDate = lossDate;
	}
	public String getLossCd() {
		return lossCd;
	}
	public void setLossCd(String lossCd) {
		this.lossCd = lossCd;
	}
	public String getLossAbbr() {
		return lossAbbr;
	}
	public void setLossAbbr(String lossAbbr) {
		this.lossAbbr = lossAbbr;
	}
	public String getInsuredDesc() {
		return insuredDesc;
	}
	public void setInsuredDesc(String insuredDesc) {
		this.insuredDesc = insuredDesc;
	}
	public String getCurrencyCd() {
		return currencyCd;
	}
	public void setCurrencyCd(String currencyCd) {
		this.currencyCd = currencyCd;
	}
	public String getInsuredClm() {
		return insuredClm;
	}
	public void setInsuredClm(String insuredClm) {
		this.insuredClm = insuredClm;
	}
	public String getApprovedAmt() {
		return approvedAmt;
	}
	public void setApprovedAmt(String approvedAmt) {
		this.approvedAmt = approvedAmt;
	}
	public String getLossOsAmt() {
		return lossOsAmt;
	}
	public void setLossOsAmt(String lossOsAmt) {
		this.lossOsAmt = lossOsAmt;
	}
	public String getLossPdAmt() {
		return lossPdAmt;
	}
	public void setLossPdAmt(String lossPdAmt) {
		this.lossPdAmt = lossPdAmt;
	}
	public String getAdjFeeAmt() {
		return adjFeeAmt;
	}
	public void setAdjFeeAmt(String adjFeeAmt) {
		this.adjFeeAmt = adjFeeAmt;
	}
	public String getClmStatCd() {
		return clmStatCd;
	}
	public void setClmStatCd(String clmStatCd) {
		this.clmStatCd = clmStatCd;
	}
	public String getClmStatDesc() {
		return clmStatDesc;
	}
	public void setClmStatDesc(String clmStatDesc) {
		this.clmStatDesc = clmStatDesc;
	}
	public String getAdjIdParam() {
		return adjIdParam;
	}
	public void setAdjIdParam(String adjIdParam) {
		this.adjIdParam = adjIdParam;
	}
	public String getClmStatParam() {
		return clmStatParam;
	}
	public void setClmStatParam(String clmStatParam) {
		this.clmStatParam = clmStatParam;
	}
	public String getDateParam() {
		return dateParam;
	}
	public void setDateParam(String dateParam) {
		this.dateParam = dateParam;
	}
	public String getDateRange() {
		return dateRange;
	}
	public void setDateRange(String dateRange) {
		this.dateRange = dateRange;
	}
	public String getDateFrom() {
		return dateFrom;
	}
	public void setDateFrom(String dateFrom) {
		this.dateFrom = dateFrom;
	}
	public String getDateTo() {
		return dateTo;
	}
	public void setDateTo(String dateTo) {
		this.dateTo = dateTo;
	}
	@Override
	public String toString() {
		return "CLMR010C [extractUser=" + extractUser + ", extractDate=" + extractDate + ", adjId=" + adjId
				+ ", adjName=" + adjName + ", claimId=" + claimId + ", claimNo=" + claimNo + ", policyId=" + policyId
				+ ", policyNo=" + policyNo + ", cedingId=" + cedingId + ", cedingName=" + cedingName + ", coRefNo="
				+ coRefNo + ", lossDate=" + lossDate + ", lossCd=" + lossCd + ", lossAbbr=" + lossAbbr
				+ ", insuredDesc=" + insuredDesc + ", currencyCd=" + currencyCd + ", insuredClm=" + insuredClm
				+ ", approvedAmt=" + approvedAmt + ", lossOsAmt=" + lossOsAmt + ", lossPdAmt=" + lossPdAmt
				+ ", adjFeeAmt=" + adjFeeAmt + ", clmStatCd=" + clmStatCd + ", clmStatDesc=" + clmStatDesc
				+ ", adjIdParam=" + adjIdParam + ", clmStatParam=" + clmStatParam + ", dateParam=" + dateParam
				+ ", dateRange=" + dateRange + ", dateFrom=" + dateFrom + ", dateTo=" + dateTo + "]";
	}
}
